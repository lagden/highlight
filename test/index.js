...
test('Parametros opcionais', t => {
	const source = 'My name is Bond, James Bond...'
	const marked = highlight(source, 'is Bond', false)
	t.is(marked, 'My name <mark>is Bond</mark>, James Bond...')
})
...
test('[Case sensitive] Deve retornar o texto com as palavras "B" e "b" marcadas com a tag <mark>', t => {
	...
	t.is(marked, 'Hey Brow!! Tudo <mark>b</mark>lz...')
})
...
test('[Case sensitive 2] Deve retornar o texto com as palavras "B" e "b" marcadas com a tag <mark>', t => {
	...
	const marked = highlight(source, 'B')
	t.is(marked, 'Tudo blz, <mark>B</mark>row!! <mark>B</mark>asta você bater na minha mão...')
})
...
test('Bug output', t => {
	...
	const marked = highlight(source, 'are to a is in on the of at or an', '<b>$&</b>')
...
test('Numbers', t => {
	...
	const marked = highlight(source, 'am 1', '<b>$&</b>')
